...
export const editProfile = async (
      ...
      errors: [
        {
          field: "general",
          message:
            "At least one field (name, lastname, lastname2, phone, password) is required",
        },
      ],
    });
  }
  ...
  if (
    name &&
    !/^(?=(?:.*[A-Za-zÁÉÍÓÚáéíóúñÑ]){2,})[A-Za-zÁÉÍÓÚáéíóúñÑ ]+$/.test(name)
  ) {
    errors.push({
      field: "name",
      message:
        "Name must be at least 2 letters and can only contain letters and spaces",
    });
  }

  if (
    lastname &&
    !/^(?=(?:.*[A-Za-zÁÉÍÓÚáéíóúñÑ]){2,})[A-Za-zÁÉÍÓÚáéíóúñÑ ]+$/.test(lastname)
  ) {
    errors.push({
      field: "lastname",
      message:
        "Lastname must be at least 2 letters and can only contain letters and spaces",
    });
  }

  if (
    lastname2 &&
    !/^(?=(?:.*[A-Za-zÁÉÍÓÚáéíóúñÑ]){2,})[A-Za-zÁÉÍÓÚáéíóúñÑ ]+$/.test(
      lastname2,
    )
  ) {
    errors.push({
      field: "lastname2",
      message:
        "Lastname2 must be at least 2 letters and can only contain letters and spaces",
    });
  }
  ...
  if (wantsToChangePassword) {
    if (!password || !new_password) {
      errors.push({
        field: "password",
        message:
          "Both password and new_password are required to change password",
      });
    } else {
      const passwordRegex =
        /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[^\s¡¿"ºª·`´çñÑ,]).{8,}$/;
      if (!passwordRegex.test(new_password)) {
        errors.push({
          field: "new_password",
          message:
            "Password must be at least 8 characters long, contain one number, one uppercase letter, one lowercase letter, no spaces, and no special characters like '\\¡¿\"ºª·`´çñÑ,'",
        });
      }
    }
  }
  ...
  try {
    ...
    return res
      .status(500)
      .json({ errors: [{ field: "server", message: "Server error" }] });
  }
};
...
export const addAddress = async (req: Request, res: Response): Promise<any> => {
  ...
  if (!address || typeof address !== "object") {
    ...
    if (via && via.trim().length < 2)
      errors.push({
        field: "via",
        message: "Via must be at least 2 characters if provided",
      });

    if (primary_number && primary_number.trim().length < 1)
      errors.push({
        field: "primary_number",
        message: "Primary number must be at least 1 character if provided",
      });

    if (complement_1 && complement_1.trim().length < 1)
      errors.push({
        field: "complement_1",
        message: "Complement 1 must be at least 1 character if provided",
      });

    if (complement_2 && complement_2.trim().length < 1)
      errors.push({
        field: "complement_2",
        message: "Complement 2 must be at least 1 character if provided",
      });

    if (!neighborhood || neighborhood.trim().length < 2)
      errors.push({
        field: "neighborhood",
        message: "Neighborhood is required and must be at least 2 characters",
      });

    if (reference && reference.trim().length < 2)
      errors.push({
        field: "reference",
        message: "Reference must be at least 2 characters if provided",
      });
  }
  ...
  try {
    ...
    const normalized = {
      ...
      via: normalize(address.via),
      primary_number: normalize(address.primary_number),
      complement_1: normalize(address.complement_1),
      complement_2: normalize(address.complement_2),
      neighborhood: address.neighborhood.trim().toLowerCase(),
      reference: normalize(address.reference),
    };
    ...
    const exists = user.addresses.some(
      (existing) =>
        ...
        (existing.via?.toLowerCase() || null) === normalized.via &&
        (existing.primary_number?.toLowerCase() || null) ===
          normalized.primary_number &&
        (existing.complement_1?.toLowerCase() || null) ===
          normalized.complement_1 &&
        (existing.complement_2?.toLowerCase() || null) ===
          normalized.complement_2 &&
        existing.neighborhood.toLowerCase() === normalized.neighborhood &&
        (existing.reference?.toLowerCase() || null) === normalized.reference,
    );
    ...
    const newAddress = Address.create({
      ...
      via: address.via?.trim() || null,
      primary_number: address.primary_number?.trim() || null,
      complement_1: address.complement_1?.trim() || null,
      complement_2: address.complement_2?.trim() || null,
...
export const toggleFavoriteAddress = async (
  req: Request,
  res: Response,
): Promise<any> => {
  ...
  try {
    ...
    if (!address) {
      return res.status(404).json({
        errors: [
          { field: "address", message: "Address not found for this user" },
        ],
      });
    }
